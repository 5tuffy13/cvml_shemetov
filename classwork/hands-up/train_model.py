...
def detect_hands_up(annotated, keypoints):
    ...
    if nose_seen and eyes_seen:
        if (left_shoulder[1] > left_elbow[1] and left_elbow[1] > left_wrist[1]
            and
            right_shoulder[1] > right_elbow[1] and right_elbow[1] > right_wrist[1]):
            
            cv2.putText(annotated, "Hands up !!!", (20,80), cv2.FONT_HERSHEY_SIMPLEX, 4, (0,255,0), 3)
            cv2.putText(annotated, f"{left_angle:.1f} degrees", (20,180), cv2.FONT_HERSHEY_SIMPLEX, 4, (0,255,0), 3)
            return True
...
while camera.isOpened():
    ...
    if detect_hands_up(annotated, keypoints[0]):
        if ps is None:
            ps = playsound("./classwork/hands-up/sound.mp3", block=False)
        else:
            if not ps.is_alive():
                ps = None
