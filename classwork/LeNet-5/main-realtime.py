...
while True:
    ...
    with torch.no_grad():
        ...
        print(prediction, (probability * 100).astype("uint8").max())
    text = canvas.copy()
    print(prediction)
    if (probability * 100).astype("uint8").max() > 85:
        cv2.putText(text, f"{prediction} {(probability * 100).astype("uint8").max()}%", (30, 30),cv2.FONT_HERSHEY_SIMPLEX, 1, 255, 1)
